...
// API Base URL
const API_BASE = ""
const TASK_TYPE = window.TASK_TYPE || "UNIVERSITY"
...
function openTaskModal(task = null) {
  ...
  if (task) {
    title.textContent = "Editar Tarefa Acadêmica"
    fillTaskForm(task)
  } else {
    title.textContent = "Nova Tarefa Acadêmica"
...
async function deleteTask(taskId) {
  if (!confirm("Tem certeza que deseja excluir esta tarefa acadêmica?")) return
...
async function loadTasks() {
  try {
    ...
    renderKanbanBoard(getMockUniversityTasks())
  }
}

function getMockUniversityTasks() {
  return [
    {
      id: 1,
      title: "Estudar para Prova de Cálculo",
      description: "Revisar derivadas e integrais",
      status: "TO_DO",
      priority: "HIGH",
      type: "UNIVERSITY",
      due_date: "2024-07-15",
...
function createTaskElement(task) {
  ...
  const tags = task.tags
    ? task.tags
        .split(",")
        .map((tag) => `<span class="task-tag university-tag">${tag.trim()}</span>`)
        .join("")
    : ""
